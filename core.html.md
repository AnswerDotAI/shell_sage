...
def get_sage(
    ...
    prompt_cache_key:NoneType=None, # Stable conversation key for OpenAI Responses; generated once if omitted
...
def get_res(
    ...
    tools:NoneType=None, parallel_tool_calls:NoneType=None, prompt_cache_key:NoneType=None,
    reasoning_effort:NoneType=None, web_search_options:NoneType=None, cache_idxs:NoneType=None, ttl:NoneType=None
):
```

*One model turn plus its tool round, recursing for the next;
turn-constant options come from `turn_opts`*

Queries and their context are literal user text, even when a diff or
terminal history contains fastllm tool or usage markers. They must not
be parsed as assistant history.
...
async def main(
    ...
    v:str='%(prog)s 1.1.4', # Print version
