...
# %% auto #0
__all__ = ['console', 'print', 'sp', 'ssp', 'default_cfg', 'tools', 'sps', 'log_path', 'AsyncChat', 'get_pane', 'get_panes',
...
def get_sage(model, mode='default', search=False, use_safecmd=False, backend='lisette', vendor_name=None):
    t = tools + [bash] if use_safecmd else tools
    if use_safecmd: _always_allow.add('bash')
    return AsyncChat(model=model, sp=sps[mode], tools=t, search=search, backend=backend, vendor_name=vendor_name)

# %% ../nbs/00_core.ipynb #2788faf4
async def get_res(sage, q, opts):
    global _res
    _res = ""
    kw = dict(max_steps=10, stream=True, api_base=opts.api_base, api_key=opts.api_key, think=opts.think)
    if opts.backend == 'fastllm': kw['base_url'] = kw.pop('api_base')
    else: from litellm.types.utils import ModelResponseStream
    async for chunk in await sage(q, **kw):
        if opts.backend == 'fastllm' and isinstance(chunk, dict) and 'text' in chunk:
            _res += chunk['text']
        elif opts.backend == 'lisette' and isinstance(chunk, ModelResponseStream): 
            _res += chunk.choices[0].delta.content or ""
        else: continue
        yield _res
...
async def main(
    ...
    opts = get_opts(history_lines=history_lines, model=model, search=search,
                    ...
                    backend=backend, vendor_name=vendor_name, log=None)
    ...
    try:
        with Live(Spinner("dots", text="Connecting..."), auto_refresh=False) as live:
            ...
            sage = get_sage(opts.model, mode, search=opts.search, use_safecmd=opts.safecmd, backend=opts.backend, vendor_name=opts.vendor_name)
