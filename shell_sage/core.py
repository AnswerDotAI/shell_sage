...
# %% auto #0
__all__ = ['console', 'print', 'sp', 'ssp', 'GHOSTTY_SCROLLBACK_SCRIPT', 'default_cfg', 'tools', 'sps', 'log_path', 'pane_mark',
           'get_pane', 'get_panes', 'tmux_history_lim', 'get_ghostty_history_macos', 'is_ghostty', 'get_hist_tmux',
           'get_hist_osa', 'get_history', 'get_ghostty_history', 'get_terminal_history', 'get_opts', 'with_permission',
           'rg', 'ls', 'fd', 'get_sage', 'get_res', 'Log', 'mk_db', 'main', 'extract_cf', 'extract']
...
# %% ../nbs/00_core.ipynb #73c88c39
def _tmux_fmt(fmt, pid=None):
    cmd = ['tmux', 'display-message', '-p']
    if pid: cmd += ['-t', pid]
    return co(cmd+[fmt], text=True).strip()

def pane_mark(pid=None):
    "Absolute history line recorded by the `ssage_clear` shell binding, or None"
    if not pid: pid = _tmux_fmt('#{pane_id}')
    f = Path('~/.cache/shell_sage').expanduser()/f"mark-{pid.lstrip('%')}"
    try: return int(f.read_text())
...
def get_pane(n, pid=None):
    ...
    mark = pane_mark(pid)
    if mark is not None: n = min(n, max(int(_tmux_fmt('#{history_size}', pid)) - mark, 0))
