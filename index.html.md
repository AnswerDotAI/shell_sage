...
[![PyPI
version](https://badge.fury.io/py/shell-sage.svg)](https://badge.fury.io/py/shell-sage)
[![Python
3.8+](https://img.shields.io/badge/python-3.10+-blue.svg)](https://www.python.org/downloads/)
[![License:
...
Ctrl-L looks like it clears the terminal, but it only wipes the visible
screen – tmux’s history still holds everything, so ShellSage keeps
seeing it. The
[`ssage_clear`](https://github.com/AnswerDotAI/shell_sage/blob/main/ssage_clear.sh)
binding gives ctrl-L real “clear” semantics for ShellSage without
destroying anything: it scrolls the screen into history and records a
mark, and ShellSage captures never reach past the most recent clear,
...
The binding is only active at the shell prompt, so full-screen apps
still receive a plain ctrl-L, and outside tmux it falls back to a normal
clear.

### Piping Content for Analysis

One of ShellSage’s most powerful features is analyzing piped input.
Piped content and terminal history are sent as literal user text,
including any embedded tool or usage markers:
...
and OpenAI model names. For other providers, prefix the model name with
the vendor, e.g. `openrouter/openai/gpt-5`:
...
<td><code>CODEX_AUTH_TOKEN</code> or
<code>~/.codex/auth.json</code></td>
</tr>
<tr>
<td><code>claude_code</code></td>
<td>Claude Code subscription (no key needed)</td>
</tr>
<tr>
...
Prefix the model with a vendor when the name does not identify the
provider on its own:
...
A `claude_code/` prefix uses your Claude Code subscription instead of an
API key:
...
For another OpenAI-compatible API, provide the API base URL, API key,
and model:
...
For known providers, prefer the matching prefix from the table above so
fastllm can use the right API format and environment variable.
...
ShellSage can execute shell commands on your behalf using the
[safecmd](https://github.com/AnswerDotAI/safecmd) library, which
provides a safety layer via an allow-list approach.
...
- Can be customized to allow or restrict allowed commands/destinations
...
> See the [safecmd
> documentation](https://github.com/AnswerDotAI/safecmd) for details on
> the allow-list
